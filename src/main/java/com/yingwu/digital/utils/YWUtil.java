...
public class YWUtil {
    ...
    private static final Map<Integer, YWUtil> instanceMap = new HashMap<Integer, YWUtil>();

    // 必须提供正确的参数,以保证 suffix 在集群环境的唯一性
    private YWUtil(final int suffix) {
        YWUtil.suffix = suffix;
    }
     ...
     * @return 主键生成器实例
     */
    public synchronized static YWUtil getInstance() {
     ...
     * @return 主键生成器实例
     */
    public synchronized static YWUtil getInstance(int suffix) {
        YWUtil pkgen;
        if (!instanceMap.containsKey(suffix)) {
            pkgen = new YWUtil(suffix);
